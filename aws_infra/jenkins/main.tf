resource "aws_instance" "aws05_jenkins_server" {
  ami           = var.ami_id
  ...
  subnet_id = data.aws_subnets.aws05_private_subnets.ids[0]
  ...
  iam_instance_profile = data.aws_iam_instance_profile.aws05_ec2_profile.name
...
resource "aws_lb_target_group_attachment" "aws05_jenkins_attach" {
  target_group_arn = data.aws_lb_target_group.aws05_jenkins_tg.arn
